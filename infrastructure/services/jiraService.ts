...
import type { JiraConfig, JiraIssue, WorklogRequest, WorklogResponse } from "core/types/jira.types";
...
export class JiraService {
   ...
   */
  async logWork(issueKey: string, worklog: WorklogRequest): Promise<WorklogResponse> {
    try {
   ...
   */
  async getAssignedIssues(issueTypes: string[] = ['Bug', 'Story', 'Task']): Promise<JiraIssue[]> {
    try {
      const jql = `assignee = currentUser() AND issuetype in (${issueTypes.map(t => `"${t}"`).join(',')})`;
